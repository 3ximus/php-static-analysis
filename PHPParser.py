...
class PatternCollection(object):
	...
	def __init__(self, path):
		...
		with open(path, 'r') as fp:
			self.parse_patterns_file(fp)

	def parse_patterns_file(self, fp):
		lines = fp.readlines()
...
class PHPParser(object):
	...
	def __init__(self, path, pattern):
		...
		if not os.path.exists(path):
			...
			sys.exit(-1)
		with open(path, 'r') as fp:
			self.parse_PHP_file(fp) # builds node graph

# -------- FILE PARSE METHOD --------

	def parse_PHP_file(self, fp):
		'''Reades file creating nodes and adding them to the flow_graph'''
		line = ""
		lineno = -1
...
# -------- PARSE METHODS --------
	...
	def process_pattern(self, line, lineno, var_node=None):
		...
		if match_name:
			if match_type == Pattern.ENTRY_POINT:
				...
				return self.process_end_none(line, match_name, match_type, lineno)
		return None
	...
	def process_var_assignment(self, match, lineno):
		...
		if not self.process_pattern(match.group(2), lineno, var_node=var_node):
			...
			if string_match:
				str_node = self.process_string(match.group(2), lineno)
				if str_node:
					self.flow_graph.add_node(var_node, str_node)
			else:  # string didnt match, try variable to variable assignment
				var_match = self.PHP_VARIABLE.search(match.group(2))
				node_var = self.find_nodes_by_value(var_match.group(0))
				if node_var != []:
	...
	def process_end_none(self, match, match_name, match_type, lineno):
		...
		p_node = self.process_pattern(arg_match, lineno)
		if p_node:
			parent_nodes = [p_node,]
		...
		if parent_nodes != [] and match_type == Pattern.SANITIZATION_FUNCTION:
			if VERBOSE: print "%s\tEndNode: %s%s" % (COLOR.GREEN, COLOR.NO_COLOR, match_name)
			...
			if VERBOSE: print "%s\tEndNode: %s%s" % (COLOR.RED, COLOR.NO_COLOR, match_name)
	...
	def process_string(self, match, lineno):
		...
		parent_nodes = self.find_nodes_by_value(*args)
		if parent_nodes != []:
			str_node = StringNode(match, lineno)
			self.flow_graph.add_node(str_node, *parent_nodes)
			return str_node
...
# -------- OTHER METHODS --------
	...
	def annotate_line(self, lineno, anotation, mark_inline_type=False):
		'''Inserts an anotation on a certain snippet line, mark_inline_type is a boolean used to do inline annotations'''
		if mark_inline_type:
	...
	def get_processed_file(self, in_line_annotations=False):
		...
		if not self.processed_file:
			for node in self.flow_graph.walk_top_down(self.flow_graph.end_nodes):
				if isinstance(node, VarNode) and node.entry_point:
					...
					self.annotate_line(node.lineno, COLOR.GREEN+"<- Sanitization Function"+COLOR.NO_COLOR, in_line_annotations)
...
class VariableFlowGraph(object):
...
# -------- OUTPUT METHODS --------
	...
	def _internal__repr__(self, nodes, trace_count, span):
		temp_out = ""
		if len(nodes) != 1: span.append(trace_count)
		for i, node in enumerate(nodes):
			if len(nodes) == i+1 and trace_count in span: span.remove(trace_count)
			temp_out += "\n%s%s%s%s%s%s%s" %  \
...
# -------- NODES METHODS --------
	...
	def add_node(self, node, *parentNodes):
		...
		for p_node in parentNodes:  # if there arent any parent nodes this cycle wont run
			if not self.has_node(p_node):
				print "Shouldn\'t happen."
				continue # node doesnt exist in the tree
			if p_node in self.end_nodes:
				continue # cant add nodes after end_nodes
			p_node.next.append(node)
			node.prev.append(p_node)
	...
	def walk_bottom_up(self, nodes):
		'''Iterate over this method to retrieved all the nodes in the tree one by one '''
		for n in nodes:
			if n: yield n
			for x in self.walk_bottom_up(n.next):
				if x: yield x
...
class StringNode(Node):
	def __init__(self, value, lineno):
		super(StringNode, self).__init__("str%d" % get_next_int(), value, lineno)
	def __repr__(self):
		return "[ %s ]%s" % (self.nid, (" - "+self.value if VERBOSE else ""))

class VarNode(Node):
	def __init__(self, value, lineno, entry_point=False):
...
class EndNode(Node):
	def __init__(self, value, lineno, poisoned):
		super(EndNode, self).__init__("end%d" % get_next_int(), value, lineno)
...
class EntryNode(Node):
	def __init__(self, value, lineno):
		super(EntryNode, self).__init__("entry%d" % get_next_int(), value, lineno)
	def __repr__(self):
		return "[ %s ]" % self.value
